...
app.post('/api/locations', authenticateToken, uploadLocationImages, async (req, res) => {
    try {
        ...
        const location = {
            ...
            submittedBy: {
                ...
                profilePicture: submitter.profilePicture
            },
...
app.get('/api/locations', async (req, res) => {
    try {
        ...
        // Only show verified locations to guest users (no auth header)
        const authHeader = req.headers['authorization'];
        if (!authHeader) {
            locations = locations.filter(loc => loc.status === 'verified');
        } else if (status) {
        ...
        const locationsWithRatings = locations.map(location => {
            ...
            const submitter = users.find(u => u.id === location.submittedBy._id);
            
            return {
                ...location,
                submittedBy: {
                    _id: location.submittedBy._id,
                    username: location.submittedBy.username,
                    profilePicture: submitter ? submitter.profilePicture : null
                },
